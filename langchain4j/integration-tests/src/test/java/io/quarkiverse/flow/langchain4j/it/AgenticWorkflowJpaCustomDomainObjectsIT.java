...
public class AgenticWorkflowJpaCustomDomainObjectsIT {
    ...
    /**
     * Register custom domain classes for AgenticScope deserialization.
     * <p>
     * WORKAROUND: Until quarkus-langchain4j provides automatic configuration for AgenticScope
     * type allowlisting, applications must manually register their domain classes that will be
     * stored in AgenticScope state.
     * <p>
     * This is required because langchain4j 1.20.0+ uses a PolymorphicTypeValidator that denies
     * deserialization of non-allowlisted types for security. The classloader fix from
     * langchain4j PR #6207 allows finding the classes, but they must still be explicitly allowed.
     *
     * @see <a href="https://github.com/langchain4j/langchain4j/pull/6207">langchain4j PR #6207</a>
     */
    @BeforeAll
    static void registerAgenticScopeDomainTypes() {
        // Register test domain classes
        AgenticScopeSerializer.registerForDeserializationPackageOf(TripItinerary.class);
        // Note: registerForDeserializationPackageOf registers the entire package, so
