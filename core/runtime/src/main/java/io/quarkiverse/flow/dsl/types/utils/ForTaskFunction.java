...
public class ForTaskFunction {
    ...
    public static <T, V> ForTask withCollection(
        ...
        return withCollection(forTask, collection,
                (Class<T>) ReflectionUtils.methodType(collection).map(m -> m.parameterType(0)).orElse(null));
    }
    ...
    public static <T, V> ForTask withCollection(
        ...
        if (forConfig.getIn() == null) {
            forConfig.setIn(new In().withForInInlineArray(List.of(new ForIn().withAdditionalProperty(COLLECTION, collection)
