...
class TokenAuthPolicyExtractorTest {
    ...
    void extract_named_oauth2_policy() {
        Workflow workflow = FlowWorkflowBuilder.workflow("orders")
    ...
    void extract_http_with_named_policy() {
        Workflow workflow = FlowWorkflowBuilder.workflow("orders")
                ...
                .tasks(FlowDSL.call(
                        FlowDSL.http("payment")
                                .get()
                                .uri(URI.create("https://api.example.com"), FlowDSL.use("keycloak"))))
                .build();
        ...
        Workflow workflow = FlowWorkflowBuilder.workflow("orders")
                ...
                .tasks(
                        FlowDSL.call(FlowDSL.http("payment")
                                .uri(URI.create("https://api1.example.com"), FlowDSL.use("keycloak"))),
                        FlowDSL.call(FlowDSL.http("shipping")
                                .uri(URI.create("https://api2.example.com"), FlowDSL.use("auth0"))))
                .build();
    ...
    void extract_empty_workflow() {
        Workflow workflow = FlowWorkflowBuilder.workflow("empty").build();
    ...
    void extract_workflow_without_token_auth() {
        Workflow workflow = FlowWorkflowBuilder.workflow("basic-auth")
                .use(use -> use.authentications(auth -> auth.authentication("basic", r -> r.basic(
                        basic -> basic.username("user").password("pass")))))
                .tasks(FlowDSL.call(
                        FlowDSL.http("api")
                                .uri(URI.create("https://api.example.com"), FlowDSL.use("basic"))))
                .build();
        ...
        Workflow workflow = FlowWorkflowBuilder.workflow("orders")
                .tasks(FlowDSL.call(
                        FlowDSL.http("payment")
                                .post()
                                .uri(URI.create("https://api.example.com/payment"),
                                        FlowDSL.oauth2("https://auth.example.com", CLIENT_CREDENTIALS, "${ $secret.client_id }",
                                                "secret",
                                                e -> e.token("/oauth2/token")))))
                .build();
