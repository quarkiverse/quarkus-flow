...
<3> Use `FlowDSL.output()` to get type-safe access to the current task's output
<4> Use `FlowDSL.input()` to get type-safe access to original workflow input
...
* Use **`FlowDSL.input(workflowContext, Type.class)`** for type-safe access to the original workflow input
* Use **`FlowDSL.output(taskContext, Type.class)`** for type-safe access to the current task's output
