...
While this cookbook provides extensive examples for defining workflows using the CNCF Serverless Workflow YAML DSL, Quarkus Flow *strongly recommends the Java DSL* as the primary method for defining workflows. The Java DSL offers superior type safety, refactoring capabilities, and seamless IDE integration.
...
When calling secure endpoints, you can define the authentication mechanism inline. The DSL supports Bearer tokens, OIDC, and OAuth2.
