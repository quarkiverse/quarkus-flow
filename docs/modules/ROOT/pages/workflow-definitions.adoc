...
This guide shows you how to load link:https://github.com/serverlessworkflow/specification/blob/main/dsl-reference.md[CNCF Workflow definitions] (YAML DSL) into your Quarkus application and expose them as `WorkflowDefinition`/`Flow` CDI beans you can inject and execute.
...
[NOTE]
====
Quarkus Flow extension generates `WorkflowDefinition` and `Flow` beans based on the content of the YAML files.
More specifically, the `document.namespace` and `document.name` fields are used to create unique identifiers for each workflow.

There are two types of identifiers involved:

* *Regular:* `\{namespace}:\{name}`.
* *Fully Qualified Class Name:* `io.quarkiverse.flow.generated.flow.\{namespace}.\{name}Workflow`.
...
[TIP]
====
You can change the base namespace (`io.quarkiverse.flow.generated`) using the xref:configuration.adoc#quarkus-flow_quarkus-flow-definitions-namespace[`quarkus.flow.definitions.namespace`] property in your `application.properties`.
====
