...
Workflow files can also be contributed by JAR dependencies. Any dependency on the application classpath that contains the configured definitions directory (default `flow/`) has its workflow files discovered at build time, exactly as if they were part of the application:
