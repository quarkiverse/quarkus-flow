...
NOTE: If you also use xref:structured-logging.adoc[Structured Logging] for workflow events alongside `quarkus-logging-json`, Quarkus Flow automatically routes those events to a separate file handler with pure JSON formatting. This safely isolates your event streams and prevents double JSON serialization without any extra configuration. See xref:structured-logging.adoc#integration-with-quarkus-logging-json[Integration with quarkus-logging-json] for details.
