...
TIP: For HTTP client timeouts, TLS, proxies and connection pooling, see the dedicated xref:http-client.adoc[HTTP client configuration] page.

== HTTP tasks

HTTP calls are represented in the CNCF Workflow Spec as link:https://github.com/serverlessworkflow/specification/blob/main/dsl-reference.md#http-call[**HTTP Call tasks**]. In Quarkus Flow you normally interact with them through:
...
Under the hood they all produce a HTTP call task executed by the same HTTP client.

=== Simple GET/POST with Func DSL
...
=== Using the low‑level HTTP builder
...
=== Calling operations by `operationId`
...
For even richer tracing (including MDC with workflow instance id, task id, etc.) see the dedicated xref:tracing.adoc[Tracing] documentation.

== HTTP errors and `WorkflowException` mapping

When an HTTP or OpenAPI task fails (4xx/5xx, timeouts, etc.), the workflow engine throws a `WorkflowException` containing a **`WorkflowError`**. `WorkflowError` follows the link:https://github.com/serverlessworkflow/specification/blob/main/dsl-reference.md#error[CNCF Workflow Spec error model], which is aligned with **RFC 7807 Problem Details**:
...
* status code is `error.status` (e.g. `401`), and
* body is the `WorkflowError` serialized as JSON (Problem Details style).

=== CompletionStage vs blocking style

How this surfaces in your REST resources depends on how you call the workflow:

* If you return a **`CompletionStage`** from your JAX‑RS endpoint and call `instance(...).start()` without blocking, any `WorkflowException` will propagate directly and be handled by the mapper.
* If you choose the **blocking style** (`.start().get()`), Java wraps the underlying exception in `ExecutionException`. In that case you should either:

  * unwrap and rethrow the `WorkflowException`, or
  * prefer the `CompletionStage` style so the mapper can see the original exception.
