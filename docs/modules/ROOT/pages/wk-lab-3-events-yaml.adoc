...
To enable event-driven capabilities, you need Quarkus Flow Messaging and a messaging connector. For this lab, we will use the Kafka connector. Thanks to Quarkus Dev Services, you don't need a local Kafka instance; Quarkus will spin one up for you in a container automatically.
...
Create a new file at `src/main/resources/flow/uppercase-message.yaml`. This workflow acts as a "Transformer" that listens for a specific event type, modifies the data using a `jq` expression, and emits a result event.

[source,yaml]
.src/main/resources/flow/uppercase-message.yaml
----
document:
  dsl: '1.0.0'
  namespace: demo
...
do:
  - setUpper:
      set:
        message: "${ .[0].data.message | ascii_upcase }"
  - emitResult:
      emit:
        event:
          with:
