...
== 5. jq Security Best Practices

`jq` expressions in Quarkus Flow are part of the workflow definition itself. They
are loaded from trusted YAML, JSON, or Java files when the workflow is parsed, then
evaluated against runtime data as plain JSON values.

That means untrusted runtime input does not become executable `jq` code. For
example, if a workflow contains:
...
and the caller sends `{"userInput":"${ system(\"rm -rf /\") }"}`, Quarkus Flow
treats that value as a literal string. It is data flowing through a pre-defined
expression, not a new expression compiled at runtime.

[IMPORTANT]
Workflow definition files should still be treated as trusted source code. Apply
version control, code review, and access control to them just as you would for
application code.

Complex regular expressions inside trusted workflow definitions can still be
expensive to evaluate. When you process untrusted or high-volume input, keep
task timeout budgets explicit instead of relying on defaults. The Serverless
...
=== jq Timeout recommendations

As a practical starting point:

* simple extraction and field projection: 5 to 10 seconds
* regex operations such as `match`, `test`, `gsub`, or `split`: 10 to 30 seconds
* larger transformations over sizeable payloads: 30 to 60 seconds

Tune those values to your payload size and latency budget, and pair them with
workflow-level retries only when the operation is safe to repeat.
