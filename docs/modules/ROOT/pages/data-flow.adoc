...
are loaded from trusted YAML, JSON, or Java files on your project's classpath during build time when the workflow is parsed, then
...
do:
  - validateEmail:
      set:
        isValidEmail: ${ .userInput | test("^([[:alnum:]]+[._-]?){1,20}@([[:alnum:]]+[.-]?)+[.][[:alpha:]]{2,}$") }
      timeout:
        after:
          seconds: 30
----
