...
are loaded from trusted YAML or JSON files when the workflow is parsed, then
...
expensive to evaluate. When you process untrusted or high-volume input, pair
regex-heavy `jq` expressions with sensible task and HTTP client timeout
settings.
