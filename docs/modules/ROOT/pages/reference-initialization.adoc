...
The warmup step runs after application startup. In development mode it is synchronous for immediate feedback; in production it is asynchronous so startup is not delayed. Internally, the extension emits `WorkflowApplicationReadyEvent` after warmup so supporting extensions can react to the initialized application state. We do not recommend user code listening to that event directly.
...
If `quarkus.flow.persistence.auto-restore=true`, interrupted workflows are restored automatically after the application becomes ready. The restore step looks up workflows owned by the current application ID and resumes them from their last checkpoint.
...
* Temporarily disable auto-restore with `quarkus.flow.persistence.auto-restore=false`.
* Profile startup with `-Dquarkus.profile.record-startup-method-times=true`.

=== Lease Acquisition Timeout (Kubernetes)

*Symptom*: Pod exits with code 1 and logs show `LeaseAcquisitionException` with message "Failed to acquire Kubernetes lease after PT30S. This pod cannot participate in the durable workflow pool. Exiting to allow Kubernetes to restart with a fresh lease attempt."

*Behavior*: When lease acquisition times out, the application exits immediately with exit code 1, triggering Kubernetes to restart the pod for a fresh acquisition attempt. This provides faster recovery than waiting for liveness probe failures.

*Possible causes*:

* The pod lacks RBAC permissions to manage Leases.
* All available leases in the pool are held by other pods.
* The Kubernetes API server is unreachable or slow to respond.
...
* Increase the timeout with `quarkus.flow.durable.kube.lease.member.acquire-timeout=60s`.
* Ensure the pool size matches your deployment replica count.
...
* Enable restore with `quarkus.flow.persistence.auto-restore=true`.
