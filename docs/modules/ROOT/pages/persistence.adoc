...
|===
| `indexed` (default)
| Every write also records the key in a per-instance Redis Set (`idx:<instanceId>`). Removal and restore
read that Set, so their cost scales with the instance instead of the whole keyspace. Each write pays one
...
few long-lived instances with many tasks, where removal is rare and the per-write `SADD` would be pure
overhead.
|===
