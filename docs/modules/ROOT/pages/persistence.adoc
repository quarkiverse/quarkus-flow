...
To delete a workflow instance, or to reload pending instances on startup, the Redis store must find every
key that belongs to that instance (its instance hash plus one hash per task). The
`quarkus.flow.persistence.redis.key-tracking` property selects how:

[cols="1,3"]
|===
| `indexed` (default)
| Every task write also records the task key in a per-instance Redis Set (`idx:<instanceId>`), and each
instance is created with a completeness marker (`idx:<instanceId>:sync`). Removal and restore read that
Set instead of scanning the keyspace, so their cost scales with the instance. Each task write pays one
extra `SADD`. Best for many short-lived instances or frequent instance removal.

| `scan`
| No index is kept. Removal and restore issue a keyspace `SCAN`; writes carry no extra cost. Best for a
few long-lived instances with many tasks, where removal is rare and the per-task-write `SADD` would be
pure overhead.
|===

[source,properties]
----
quarkus.flow.persistence.redis.key-tracking=scan
----

The two modes are interoperable, so you can switch without a migration step. An instance created under
`scan` has no completeness marker, so after a switch to `indexed` it keeps being resolved by `SCAN` for
the rest of its life (its index is never back-filled); only instances created under `indexed` are read
from the Set. Any `idx:` keys left behind by `indexed` are removed on the instance's deletion after
switching to `scan`.
