...
|===
| `indexed` (default)
| Every task write also records the task key in a per-instance Redis Set (`idx:<instanceId>`). Removal and
restore read that Set, so their cost scales with the instance instead of the whole keyspace. Each task
write pays one extra `SADD`. Best for many short-lived instances or frequent instance removal.
...
The two modes are interoperable, so you can switch without a migration step: an instance written under
`scan` is still found by a one-time `SCAN` after switching to `indexed`, and an `idx:` Set left behind by
`indexed` is removed on the next deletion after switching to `scan`.
