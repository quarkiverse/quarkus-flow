...
quarkus.flow.structured-logging.events=workflow.instance.*,workflow.task.failed
...
* `workflow.instance.failed` – Workflow fails with error
...
* `workflow.task.failed` – Task fails with error
...
To make tailing easier, route structured events to a separate log file:

[source,properties]
----
# Send structured logging to separate file
quarkus.log.category."io.quarkiverse.flow.structuredlogging".handlers=STRUCTURED_FILE
quarkus.log.handler.file."STRUCTURED_FILE".path=/var/log/quarkus-flow/structured-events.log
quarkus.log.handler.file."STRUCTURED_FILE".format=%s%n
----

This allows FluentBit to tail only structured events without parsing application logs.
...
* Task failures (`workflow.task.failed`)
