...
1. Enable the structured logging feature
2. Configure a log handler to capture events
...
TIP: All handler configurations shown below are **runtime-configurable**. This is essential for pre-built container images that need different logging setups per environment (dev/staging/production).
...
.Example: Switch from FILE to CONSOLE at runtime
[source,properties]
----
...
TIP: All `quarkus.log.*` properties support runtime override via environment variables. No application rebuild required!
...
- Parse application logs with the quarkus-logging-json schema
- Parse workflow events as pure JSON (no wrapper)
- Route each to different destinations (e.g., Elasticsearch for logs, PostgreSQL for events)

TIP: The `format=%s%n` setting ensures events are written as raw JSON strings without any additional formatting from `quarkus-logging-json`.
...
TIP: For CONSOLE handler, the structured events are already JSON in the log stream, so FluentBit can parse them directly without additional processing.
