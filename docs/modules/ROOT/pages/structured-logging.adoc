...
- **Event file**: Pure workflow event JSON (one event per line)
...
=== Separate Log File (Auto-Configured)

Quarkus Flow **automatically configures** a separate file handler for structured events. This is enabled by default when structured logging is turned on.

**Default behavior:**
- Events write to `target/quarkus-flow-events.log` (dev/test) or `/var/log/quarkus-flow/events.log` (production)
- Events do NOT appear in console output (preventing duplication)
- File format is raw JSON (`%s%n`)

**Why this approach?**
- ✅ Separates event streams from diagnostic logs
- ✅ Avoids double JSON serialization with `quarkus-logging-json`
- ✅ Simplifies log forwarder configuration
- ✅ Works out-of-the-box (zero configuration required)

See xref:#integration-with-quarkus-logging-json[Integration with quarkus-logging-json] for customization options.
...
[source,properties]
----
quarkus.log.handler.file."STRUCTURED_FILE".rotation.max-file-size=100M
quarkus.log.handler.file."STRUCTURED_FILE".rotation.max-backup-index=7
quarkus.log.handler.file."STRUCTURED_FILE".rotation.file-suffix=.yyyy-MM-dd
quarkus.log.handler.file."STRUCTURED_FILE".rotation.rotate-on-boot=true
----
