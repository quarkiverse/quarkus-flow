...
=== Dev/test shortcut: automatic Kafka configuration with DevServices

In dev and test mode, instead of manually enabling the bridge and configuring every `mp.messaging.*` property, you can let Quarkus Flow set up the full Kafka stack automatically with a single flag:
...
* Injects the Kafka connector (`smallrye-kafka`) on all three default channels:
** `flow-in` (inbound) — `topic=flow-in`, `StringDeserializer` / `ByteArrayDeserializer`
** `flow-out` (outbound) — `topic=flow-out`, `StringSerializer` / `ByteArraySerializer`
** `flow-lifecycle-out` (outbound) — `topic=flow-lifecycle-out`, `StringSerializer` / `ByteArraySerializer`
* Triggers Quarkus DevServices to start an ephemeral Kafka broker automatically

Quarkus DevServices start log confirms the injected configuration so you can see exactly what was set.
