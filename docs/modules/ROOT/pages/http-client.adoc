...
quarkus.flow.http.client.workflow."org.acme:myFlow:0.0.1".name=secureA

# Short workflow ID: namespace:name
quarkus.flow.http.client.workflow."org.acme:myFlow".name=secureA
...
quarkus.flow.http.client.workflow."org.acme:myFlow:0.0.1:fetchCustomers".name=secureB

# Task-level override by short workflow ID
quarkus.flow.http.client.workflow."org.acme:myFlow:fetchCustomers".name=secureB
----

**Resolution order** for a call:

. Task-level override by full/short workflow ID (`namespace:name:version:taskName` or `namespace:name:taskName`).
. Task-level override by workflow name (`workflow.<workflowName>.task.<taskName>.name`).
. Workflow-level override by full/short workflow ID (`namespace:name:version` or `namespace:name`).
. Workflow-level override by workflow name (`workflow.<workflowName>.name`).
