...
- how to shape data with **transformations**: `inputFrom`, `outputAs`, and `exportAs`.

[NOTE]
====
Source of truth for the DSL classes:

- Builder: `io.quarkiverse.flow.dsl.FlowWorkflowBuilder`
- Shortcuts (task providers): `io.quarkiverse.flow.dsl.FlowDSL` (import statically)
- Transformations: `io.quarkiverse.flow.dsl.spi.FuncTransformations` and `io.quarkiverse.flow.dsl.spi.FuncTaskTransformations`
- HTTP / OpenAPI steps: `io.quarkiverse.flow.dsl.FuncCallHttpStep`, `io.quarkiverse.flow.dsl.FuncCallOpenAPIStep`
====
...
agent("draftNewsletter", drafter::draft, Draft.class)
  .exportAs((draft, wfContext) -> {
      var current = (NewsletterContext) wfContext.currentData();
