...
* **Static policies** (no runtime expressions like `${ $secret.xxx }`) → OIDC clients registered at application startup
* **Dynamic policies** (with runtime expressions) → OIDC clients registered lazily on first use
...
[source,properties]
----
quarkus.flow.oidc."myns\:my-workflow".name=prod-auth
----
...
[source,properties]
----
quarkus.flow.oidc."myns\:my-workflow\:1.0.0".task.callSlowApi.name=slow-api-auth
----
...
[source,properties]
----
quarkus.flow.oidc.keycloak.name=prod-auth
----
...
quarkus.flow.oidc."myns\:my-workflow".name=internal-auth

# Or route a specific version
quarkus.flow.oidc."myns\:my-workflow\:2.0.0".name=internal-auth

# Or route a specific task within a versioned workflow
quarkus.flow.oidc."myns\:my-workflow\:1.0.0".task.fetchData.name=internal-auth
----
...
1. **Task-level** (most specific) — `quarkus.flow.oidc."<namespace>\:<name>\:<version>".task.<taskName>.name`
2. **Versioned workflow** — `quarkus.flow.oidc."<namespace>\:<name>\:<version>".name`
3. **Versionless workflow** — `quarkus.flow.oidc."<namespace>\:<name>".name`
4. **Named authentication** — `quarkus.flow.oidc.<authPolicyName>.name`
...
* **Static policies** (no `${ ... }` expressions): Clients registered at application startup
* **Dynamic policies** (with expressions like `${ $secret.xxx }`): Clients registered lazily on first use after expression resolution
...
Quarkus Flow creates OIDC clients through `OidcClients.newClient(...)` at two different times:

* **Static registration** (application startup): Policies without runtime expressions (`${ ... }`) are registered when the workflow is first loaded
* **Dynamic registration** (first HTTP request): Policies with expressions are registered lazily after expression resolution

Clients created this way are **not** registered with the `quarkus-oidc-client` extension: Quarkus does not track them, does not close them on shutdown, and does not schedule any background token refresh for them. Quarkus Flow therefore owns their full lifecycle:
