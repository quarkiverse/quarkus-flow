 ...
 * Workflow that:
 * <p>
 * 1. Calls an HTTP endpoint to fetch market data for a ticker.
 * 2. Pipes the result into a LangChain4j agent which writes an InvestmentMemo.
 */
@ApplicationScoped
                ...
                .tasks(
                        ...
                        get("fetchMarketData", "http://localhost:8081/market-data/{ticker}")
                                .outputAs((result, wf, tf) -> {
                                    ...
                                    // It could be a record, but we use as a Map to exemplify how to handle this type of object in the example.
                                    ...
                                    return new InvestmentPrompt(result.ticker(),
